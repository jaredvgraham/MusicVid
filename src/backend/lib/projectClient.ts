...
class ProjectClient {
    ...
    static async getProjects(clerkId: string): Promise<ProjectDocument[] | ApiError> {
        await dbConnect();
        try {
            const projects = await Project.find({ user_id: clerkId }).sort({ timeCreated: -1 });
            return projects;
        } catch (err: unknown) {
    ...
    static async getProject(projectId: string): Promise<ProjectDocument | ApiError> {
        await dbConnect();
        try {
